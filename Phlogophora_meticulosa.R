...
P_meticulosa_GBIF <- read.delim('Phlogophora_meticulosa.csv')

################species data clean##################
#convert country code from ISO2c to ISO3c
P_meticulosa_GBIF$countryCode <-  countrycode(P_meticulosa_GBIF$countryCode, origin =  'iso2c', destination = 'iso3c')
#flag problems
flags <- clean_coordinates(x = P_meticulosa_GBIF, 
...
P_meticulosa_GBIF_cl <- P_meticulosa_GBIF[flags$.summary,]
#The flagged records
P_meticulosa_GBIF_fl <- P_meticulosa_GBIF[!flags$.summary,]
#Remove records with low coordinate precision
P_meticulosa_occ  <- P_meticulosa_GBIF_cl %>%
...
P_meticulosa_occ <- P_meticulosa_occ[!is.na(P_meticulosa_occ$decimalLongitude), ]
P_meticulosa_occ <- P_meticulosa_occ[!is.na(P_meticulosa_occ$decimalLatitude), ]
P_meticulosa_occ <- P_meticulosa_occ[!is.na(P_meticulosa_occ$month), ]
write.csv(P_meticulosa_occ, file = "P_meticulosa.csv", row.names = FALSE)
P_meticulosa_occ <- read.csv("P_meticulosa.csv")

#Count the number of record in each months
num_occ <- data.frame(table(P_meticulosa_occ$month))
num_occ <- data.frame(month = month.abb, count = num_occ$Freq)
png("model_e.png",units="in", width=7, height=5,res = 300)
barplot(unlist(num_occ$count), names.arg = num_occ$month, xlab = "Month", ylab = "Count", main = "Number of P.meticulosa record in each Month", col="lightBlue")
dev.off()
...
for (month in 1:12) {
  ...
  P_meticulosa_month <- P_meticulosa_occ[P_meticulosa_occ$month == month, c("decimalLatitude", "decimalLongitude")]
  colnames(P_meticulosa_month) <- c("lat", "long")  
  # Select and rename columns, and add state column, 1 represent present
  P_meticulosa_month$state <- 1
  # Create a variable dynamically for each data set
  assign(paste0("P_meticulosa", substr(month.name[month], 1, 3)), P_meticulosa_month)
}

################spatial rarefy##########
#set folders to store outcomes of SDM built with and without land management data
dir.create("E:/Pest_SDM/P_meticulosa_newmanage")
dir.create("E:/Pest_SDM/P_meticulosa_nomanageeva")

for (month in month.abb){
  thin(loc.data = get(paste0("P_meticulosa", month)),
       lat.col =  "lat",
       long.col = "long",
       spec.col = "state",
       thin.par = 1,
       reps = 1,
       write.files = TRUE,
       out.dir = "P_meticulosa_newmanage",
       out.base = paste0("P_meticulosa", month)
  )
}
for (month in month.abb){
  assign(paste0("P_meticulosa", month), read.csv(paste0("P_meticulosa_newmanage/P_meticulosa", month, "_thin1.csv")))
}

P_meticulosayear <- data.frame()
for (month in month.abb) {
  P_meticulosayear <- rbind(P_meticulosayear, get(paste0("P_meticulosa", month)))
}
thin(loc.data = P_meticulosayear,
     lat.col =  "lat",
     long.col = "long",
     spec.col = "state",
     thin.par = 10,
     reps = 1,
     write.files = TRUE,
     out.dir = "E:/R-4.2.1-oldversion/grid_10/P_meticulosa_newmanage",
     out.base = "P_meticulosayear"
)
P_meticulosayear <- read.csv("P_meticulosayear_thin1.csv")
...
for (month in periods) {
  src_crs <- crs(elv)
  assign(paste0("P_meticulosa", month), st_as_sf(get(paste0("P_meticulosa", month)), coords=c('long', 'lat'), crs=src_crs))
  # Create a simple land mask
  land <- management1[[1]] >= 0
  # How many points to create? We'll use the same as number of observations
  n_pseudo <- nrow(get(paste0("P_meticulosa", month)))
  # Sample the points
  pseudo_dismo <- randomPoints(mask=as(land, 'Raster'), n=n_pseudo, p=st_coordinates(get(paste0("P_meticulosa", month))))
  ...
  present <- get(paste0("P_meticulosa", month))
  present$state <- 1
  absent <- pseudo_dismo
  absent$state <- 0
  ...
  assign(paste0("P_meticulosa", month), rbind(present, absent))
  #extract the environmental values for each of those points and add it into the data frame
  envt_data <- terra::extract(get(paste0("stack", month)), get(paste0("P_meticulosa", month)), fun = mean, na.rm = TRUE)
  assign(paste0("P_meticulosa", month), cbind(get(paste0("P_meticulosa", month)), envt_data))
  coordinates <- st_coordinates(get(paste0("P_meticulosa", month)))
  assign(paste0("P_meticulosa", month), data.frame(cbind(get(paste0("P_meticulosa", month)), coordinates[, c("X", "Y")])))
}
...
dir.create('P_meticulosa_newmanage/modelsscore')
dir.create('P_meticulosa_newmanage/var_importance')

# Loop through months and year-long period data to built SDMs
for (month in periods) { 
  # Format the data
  assign(paste0("P_meticulosa_", month), BIOMOD_FormatingData(
    resp.var = get(paste0("P_meticulosa", month))['state'],
    resp.xy = get(paste0("P_meticulosa", month))[, c('X', 'Y')],
    expl.var = get(paste0("P_meticulosa", month))[, c("Nfer", "Kfer", "Pfer", "Pesti", "Irri", 
                                                      "primf", "pastr", "range", "urban", "c3ann", "c3per", "c4ann", "c4per", 
                                                      "elv", "tmean", "tmean_dif", "pre", "pre_dif")],
    resp.name = paste0("P_meticulosa in ", month)
  ))
  # Perform cross-validation
  assign(paste0(month, "P_meticulosa_CV"), BIOMOD_CrossValidation(
    bm.format = get(paste0("P_meticulosa_", month)), 
    k=5, 
    nb.rep =1, 
  ...
  assign(paste0("P_meticulosa_models_", month), BIOMOD_Modeling(
    bm.format = get(paste0("P_meticulosa_", month)),
    models = c("GLM", "GBM", "CTA", "ANN", "MARS", "RF", "MAXNET"),
    data.split.table = get(paste0(month, "P_meticulosa_CV")),
    var.import = 1,
    modeling.id = paste0("demo", month),
    metric.eval = "TSS"
  ))
  # Calculate evaluation scores (TSS)
  assign(paste0("P_meticulosa_models_", month, "_scores"), get_evaluations(get(paste0("P_meticulosa_models_", month))))
  write.csv(get(paste0("P_meticulosa_models_", month, "_scores")), file = paste0("P_meticulosa_newmanage/modelsscore/P_meticulosa_modelsscore_", month, ".csv"), row.names = FALSE)
  # record variable contribution
  assign(paste0("P_meticulosa_models_", month, "_var_import"), get_variables_importance(get(paste0("P_meticulosa_models_", month))))
  var_imp_sum <- get(paste0("P_meticulosa_models_", month, "_var_import")) %>%
    group_by(full.name, rand) %>%
    summarise(total_var_imp = sum(var.imp))
  var_contribution <- get(paste0("P_meticulosa_models_", month, "_var_import")) %>%
  ...
  write.csv(get(paste0("var_con_",month)), file = paste0("P_meticulosa_newmanage/var_importance/P_meticulosa_varcon_", month, ".csv"), row.names = FALSE)
}
...
for (month in month.abb) {
  ...
  ensemble_model <- paste0("P_meticulosa_ensemble_models_", month)
  models <- paste0("P_meticulosa_models_", month)
  proj_current <- paste0("P_meticulosa_ensemble_models_", month, "_proj_current")
  proj_ssp2 <- paste0("P_meticulosa_ensemble_models_", month, "_proj_SSP2")
  proj_ssp5 <- paste0("P_meticulosa_ensemble_models_", month, "_proj_SSP5")
  ...
  write.csv(get(paste0("res_", month)), file = paste0("P_meticulosa_newmanage/P_meticulosa_rescurve_", month, ".csv"), row.names = FALSE)
  ...
  Currentname <- paste0("P_meticulosa_newmanage/Current", month, ".tif")
  writeRaster(CurrentPrediction, filename = Currentname)
  
  SSP2Prediction <- get_predictions(get(proj_ssp2))
  SSP2name <- paste0("P_meticulosa_newmanage/SSP2", month, ".tif")
  writeRaster(SSP2Prediction, filename = SSP2name)
  
  SSP5Prediction <- get_predictions(get(proj_ssp5)) 
  SSP5name <- paste0("P_meticulosa_newmanage/SSP5", month, ".tif")
  writeRaster(SSP5Prediction, filename = SSP5name)
}

# record response curve results of all months
write.csv(resvalue, "P_meticulosa_newmanage/resvalue.csv", row.names = FALSE)

###################################################################################################################
########build SDMs without land management data & get variable contributions and evaluations#######################
#set files to store model evaluation data and environmental variable importance data
dir.create('P_meticulosa_nomanageeva/modelsscore')
dir.create('P_meticulosa_nomanageeva/var_importance')

# Loop through months
for (month in periods) { 
  # Format the data
  assign(paste0("P_meticulosanom_", month), BIOMOD_FormatingData(
    resp.var = get(paste0("P_meticulosa", month))['state'],
    resp.xy = get(paste0("P_meticulosa", month))[, c('X', 'Y')],
    expl.var = get(paste0("P_meticulosa", month))[, c("primf", "pastr", "range", "urban", "c3ann", "c3per", "c4ann", "c4per", 
                                                      "elv", "tmean", "tmean_dif", "pre", "pre_dif")],
    resp.name = paste0("P_meticulosa in ", month)
  ))
  # Perform cross-validation
  assign(paste0(month, "P_meticulosanom_CV"), BIOMOD_CrossValidation(
    bm.format = get(paste0("P_meticulosanom_", month)), 
    k=5, 
    nb.rep =1, 
    do.stratification = FALSE, 
    balance = c("presences", "absences")
  ...
  assign(paste0("P_meticulosa_modelsnom_", month), BIOMOD_Modeling(
    bm.format = get(paste0("P_meticulosanom_", month)),
    models = c("GLM", "GBM", "CTA", "ANN", "MARS", "RF", "MAXNET"),
    data.split.table = get(paste0(month, "P_meticulosanom_CV")),
    modeling.id = paste0("demo", month),
    metric.eval = "TSS"
  ))
  # model evaluation
  assign(paste0("P_meticulosa_modelsnom_", month, "_scores"), get_evaluations(get(paste0("P_meticulosa_modelsnom_", month))))
  write.csv(get(paste0("P_meticulosa_modelsnom_", month, "_scores")), file = paste0("P_meticulosa_nomanageeva/P_meticulosa_modelsscore_", month, ".csv"), row.names = FALSE)
}
...
for (month in month.abb) {
  #define names of models and the variables that would be used in the loop
  ensemble_model <- paste0("P_meticulosa_ensemble_models_nomanage", month)
  models <- paste0("P_meticulosa_modelsnom_", month)
  proj_current <- paste0("P_meticulosa_ensemble_models_nomanage", month, "_proj_current")
  proj_ssp2 <- paste0("P_meticulosa_ensemble_models_nomanage", month, "_proj_SSP2")
  proj_ssp5 <- paste0("P_meticulosa_ensemble_models_nomanage", month, "_proj_SSP5")
  ...
  # Get distribution probability maps predicted by SDM incorporated with land management interventions and save as raster files
  CurrentPrediction <- get_predictions(get(proj_current))
  Currentname <- paste0("P_meticulosa_nomanageeva/Current", month, ".tif")
  writeRaster(CurrentPrediction, filename = Currentname)
  
  SSP2Prediction <- get_predictions(get(proj_ssp2))
  SSP2name <- paste0("P_meticulosa_nomanageeva/SSP2", month, ".tif")
  writeRaster(SSP2Prediction, filename = SSP2name)
  
  SSP5Prediction <- get_predictions(get(proj_ssp5)) 
  SSP5name <- paste0("P_meticulosa_nomanageeva/SSP5", month, ".tif")
  writeRaster(SSP5Prediction, filename = SSP5name)
}

####show distribution probability maps####################################################################################################
folder_path <- "P_meticulosa_newmanage/"
...
ggplot(df, aes(x = x, y = y, fill = value/1000)) +
  ...
  labs(title = "Current distribution probability of P.meticulosa in Europe", x="Longitude", y = "Latitude")
...
ggplot(df, aes(x = x, y = y, fill = value/1000)) +
  ...
  labs(title = "2040-2060 distribution probability of P_meticulosa in Europe under SSP2", x="Longitude", y = "Latitude")
...
ggplot(df, aes(x = x, y = y, fill = value/1000)) +
  ...
  facet_wrap(~ month, ncol = 4) +
  scale_fill_gradient(low="#E0D599", high = "darkblue",name = "Probability(%)", limits = c(0, 1000)) +
  labs(title = "2040-2060 distribution probability of P_meticulosa in Europe under SSP5", x="Longitude", y = "Latitude")
...
ggplot(df, aes(x = x, y = y, fill = value/1000)) +
  ...
  scale_fill_gradient(low="#E0D599", high = "darkblue",name = "Probability (%)", limits = c(0, 1)) +
  labs(title = "P.meticulosa Distribution probability  under different senarios in Jan, Jul, and Sep", x="Longitude", y = "Latitude")
...
resvalue <- read.csv("P_meticulosa_newmanage/resvalue.csv")

#get response data to Nfer
resvalueNfer <- resvalue[resvalue$expl.name == "Nfer", ]
P_meticulosastableNfer <- data.frame()
...
for(month in month.abb){
  ...
  P_meticulosastableNfer <- rbind(P_meticulosastableNfer, SN)
}
colnames(P_meticulosastableNfer)<-c("month", "pred.val", "expl.val")
P_meticulosastableNfer$pred.val <- as.numeric(P_meticulosastableNfer$pred.val)*100
P_meticulosastableNfer$expl.val <- as.numeric(P_meticulosastableNfer$expl.val)
P_meticulosastableNfer$species <- "P_meticulosa"

#get response data to Kfer
resvalueKfer <- resvalue[resvalue$expl.name == "Kfer", ]
P_meticulosahighKfer<- data.frame()
...
for(month in month.abb){
  ...
  P_meticulosahighKfer<- rbind(P_meticulosahighKfer, SN)
}
colnames(P_meticulosahighKfer)<-c("month", "pred.val", "expl.val", "range")
P_meticulosahighKfer$pred.val <- as.numeric(P_meticulosahighKfer$pred.val)*100
P_meticulosahighKfer$expl.val <- as.numeric(P_meticulosahighKfer$expl.val)
...
P_meticulosahighKfer<- P_meticulosahighKfer[!(P_meticulosahighKfer$month %in% c("Jan", "Mar","May", "Jun", "Jul", "Aug")) & P_meticulosahighKfer$range > 0.1, ]
P_meticulosahighKfer$species <- "P_meticulosa"

P_meticulosalowKfer<- data.frame()
...
for(month in month.abb){
  ...
  stableKfer <- resKfermonth[which.min(abs(resKfermonth$pred.val - stableKferp)), "expl.val"]
  SN <- c(month, num(stableKferp), num(stableKfer), num(stableKferpran))
  P_meticulosalowKfer<- rbind(P_meticulosalowKfer, SN)
}
colnames(P_meticulosalowKfer)<-c("month", "pred.val", "expl.val", "range")
P_meticulosalowKfer$pred.val <- as.numeric(P_meticulosalowKfer$pred.val)*100
P_meticulosalowKfer$expl.val <- as.numeric(P_meticulosalowKfer$expl.val)
#exclude these points with range of existence probability change < 0.1 
#If the existence probability of a species in a month varies by less than 10% as a land management factor increases, we assume that it does not significantly affect the studied environmental data (Kfer in this case)
#For some months, trend of existence probability do not show bottom values, these months are excluded
P_meticulosalowKfer<- P_meticulosalowKfer[!(P_meticulosalowKfer$month %in% c("Mar")) & P_meticulosalowKfer$range > 0.1, ]
P_meticulosalowKfer$species <- "P_meticulosa"

#get response data to Pfer
resvaluePfer <- resvalue[resvalue$expl.name == "Pfer", ]
P_meticulosaPferstarter <- data.frame()
...
for(month in month.abb){
  ...
  P_meticulosaPferstarter <- rbind(P_meticulosaPferstarter, SN)
}
colnames(P_meticulosaPferstarter)<-c("month", "pred.val", "expl.val", "range", "slope")
P_meticulosaPferstarter$pred.val <- as.numeric(P_meticulosaPferstarter$pred.val)*100
P_meticulosaPferstarter$expl.val <- as.numeric(P_meticulosaPferstarter$expl.val)
#exclude these points with range of existence probability change < 0.1 
...
P_meticulosaPferstarter <- P_meticulosaPferstarter[P_meticulosaPferstarter$range > 0.1, ]
P_meticulosaPferstarter$species <- "P_meticulosa"

P_meticulosalowPfer<- data.frame()
#extract turning points when existence probability reach the bottom
for(month in month.abb){
  resPfermonth <- resvaluePfer[resvaluePfer$month == month ,]
  #get the highest and the lowest existence probability and the range of existence probability change
  ...
  P_meticulosalowPfer<- rbind(P_meticulosalowPfer, SN)
}
colnames(P_meticulosalowPfer)<-c("month", "pred.val", "expl.val", "range")
P_meticulosalowPfer$pred.val <- as.numeric(P_meticulosalowPfer$pred.val)*100
P_meticulosalowPfer$expl.val <- as.numeric(P_meticulosalowPfer$expl.val)
#exclude these points with range of existence probability change < 0.1 
#If the existence probability of a species in a month varies by less than 10% as a land management factor increases, we assume that it does not significantly affect the studied environmental data (Pfer in this case)
P_meticulosalowPfer<- P_meticulosalowPfer[P_meticulosalowPfer$range > 0.1, ]
P_meticulosalowPfer$species <- "P_meticulosa"
P_meticulosalowPfer$slope <- 0
...
P_meticulosahighPesti <- data.frame()
...
for(month in month.abb){
  ...
  P_meticulosahighPesti <- rbind(P_meticulosahighPesti, SN)
}
colnames(P_meticulosahighPesti)<-c("month", "pred.val", "expl.val", "range")
P_meticulosahighPesti$pred.val <- as.numeric(P_meticulosahighPesti$pred.val)*100
P_meticulosahighPesti$expl.val <- as.numeric(P_meticulosahighPesti$expl.val)
#exclude these points with range of existence probability change < 0.1 
...
P_meticulosahighPesti <- P_meticulosahighPesti[P_meticulosahighPesti$range > 0.1, ]
P_meticulosahighPesti$species <- "P_meticulosa"
...
combined_plot <- grid.arrange(combined_plot, top = "Response curve of P.meticulosa in 12 months", left = "probability of pest existence", bottom = "land management variables")
plot(combined_plot)
...
for (name in hsignificantm) {
  ...
  data$month <- factor(data$month, levels = periods)
  P_meticulosastableNfer$month <- factor(P_meticulosastableNfer$month, levels = periods)
...
combined_plot <- grid.arrange(combined_plot, top = "Response curve of P.meticulosa in the 5 months that most P.meticulosa occurs", left = "probability of pest existence(%)", bottom = "land management variables")
plot(combined_plot)
...
P_meticulosastableNfer$color <- color_mapping[P_meticulosastableNfer$month]
p<-ggplot(P_meticulosastableNfer, aes(x = expl.val, y = pred.val, colour = month))+
  geom_point(aes(color=month), shape = 20, size = 4)+
  geom_point(aes(x = expl.val, y = 0, color=month), shape = 20, size = 4)+
  scale_color_manual(values = P_meticulosastableNfer$color)+
  annotate("rect", xmin = min(P_meticulosastableNfer$expl.val), xmax = max(P_meticulosastableNfer$expl.val), ymin = min(P_meticulosastableNfer$pred.val), ymax = max(P_meticulosastableNfer$pred.val),
           fill = NA, color = "red", size = 1, linetype = "dashed") +
  geom_segment(aes(x = min(expl.val), xend = max(expl.val), y = 0, yend =0, linetype= "high existence probability"), data = P_meticulosastableNfer, color = "red")+
  geom_segment(aes(x = 0, xend = 0, y = 0, yend =0, linetype= "low existence probability"), data = P_meticulosastableNfer, color = "black")+
...
sresvalue <- resvalue[resvalue$month %in% "Oct", ]
data <- sresvalue[sresvalue$expl.name == name, ]
data$month <- factor(data$month, levels = periods)
data$color <- color_mapping[data$month]
P_meticulosahighKfer$color <- color_mapping[P_meticulosahighKfer$month]
P_meticulosahighKfer$month <- factor(P_meticulosahighKfer$month, levels = periods)
P_meticulosalowKfer$color <- color_mapping[P_meticulosalowKfer$month]
P_meticulosalowKfer$month <- factor(P_meticulosalowKfer$month, levels = periods)
p<-ggplot(P_meticulosahighKfer, aes(x = expl.val, y = pred.val, colour = month))+
  geom_point(aes(color=month), shape = 20, size = 4)+
  geom_point(aes(x = expl.val, y = 0, color=month), shape = 20, size = 4)+
  scale_color_manual("month", values = color_mapping)+
  annotate("rect", xmin = min(P_meticulosahighKfer$expl.val), xmax = max(P_meticulosahighKfer$expl.val), ymin = min(P_meticulosahighKfer$pred.val), ymax = max(P_meticulosahighKfer$pred.val),
           fill = NA, color = "red", size = 1, linetype = "dashed") +
  geom_segment(aes(x = min(expl.val), xend = max(expl.val), y = 0, yend =0, linetype= "high existence probability"), data = P_meticulosahighKfer, color="red")+
  geom_point(aes(x=expl.val, y=pred.val, color=month), data=P_meticulosalowKfer, shape=20, size=4)+
  geom_point(aes(x=expl.val, y=0, color=month), data=P_meticulosalowKfer, shape=20, size=4)+
  annotate("rect", xmin = min(P_meticulosalowKfer$expl.val), xmax = max(P_meticulosalowKfer$expl.val), ymin = min(P_meticulosalowKfer$pred.val), ymax = max(P_meticulosalowKfer$pred.val),
           fill = NA, color = "black", size = 1, linetype = 2) +
  geom_segment(aes(x = min(expl.val), xend = max(expl.val), y = 0, yend =0, linetype= "low existence probability"), data = P_meticulosalowKfer, color="black")+
...
data <- sresvalue[sresvalue$expl.name == name, ]
data$month <- factor(data$month, levels = periods)
P_meticulosaPferstarter$month <- factor(P_meticulosaPferstarter$month, levels = periods)
P_meticulosalowPfer$month <- factor(P_meticulosalowPfer$month, levels = periods)
data$color <- color_mapping[data$month]
P_meticulosaPferstarter$color <- color_mapping[P_meticulosaPferstarter$month]
P_meticulosalowPfer$color <- color_mapping[P_meticulosalowPfer$month]
p<-ggplot(P_meticulosaPferstarter, aes(x = expl.val, y = pred.val, colour = month))+
  geom_point(aes(color=month), shape = 20, size = 4)+
  geom_point(aes(x = expl.val, y = 0, color=month), shape = 20, size = 4)+
  scale_color_manual(values = color_mapping)+
  annotate("rect", xmin = min(P_meticulosaPferstarter$expl.val), xmax = max(P_meticulosaPferstarter$expl.val), ymin = min(P_meticulosaPferstarter$pred.val), ymax = max(P_meticulosaPferstarter$pred.val),
           fill = NA, color = "red", size = 1, linetype = "dashed") +
  geom_segment(aes(x = min(expl.val), xend = max(expl.val), y = 0, yend =0, linetype= "high existence probability"), data = P_meticulosaPferstarter, color = "red")+
  geom_point(aes(x=expl.val, y=pred.val, color=month), data=P_meticulosalowPfer, shape=20, size=4)+
  geom_point(aes(x=expl.val, y=2, color=month), data=P_meticulosalowPfer, shape=20, size=4)+
  annotate("rect", xmin = min(P_meticulosalowPfer$expl.val), xmax = max(P_meticulosalowPfer$expl.val), ymin = min(P_meticulosalowPfer$pred.val), ymax = max(P_meticulosalowPfer$pred.val),
           fill = NA, color = "black", size = 1, linetype = 2) +
  geom_segment(aes(x = min(expl.val), xend = max(expl.val), y = 2, yend =2, linetype= "low existence probability"), data = P_meticulosalowPfer, color = "black")+
  scale_linetype_manual("turning points range",values=c("high existence probability"=2, "low existence probability"=2))+
  geom_line(aes(x = expl.val, y = pred.val*100, colour = month), data=data, linewidth = 0.8)+
  labs(x = name, y=NULL, color ="month")+
  ylim(0,80)+
...
name <- "Pesticide"
sresvalue <- resvalue[resvalue$month %in% "May", ]
data <- sresvalue[sresvalue$expl.name == name, ]
data$month <- factor(data$month, levels = periods)
P_meticulosahighPesti$month <- factor(P_meticulosahighPesti$month, levels = periods)
data$color <- color_mapping[data$month]
P_meticulosahighPesti$color <- color_mapping[P_meticulosahighPesti$month]
p<-ggplot(P_meticulosahighPesti, aes(x = expl.val, y = pred.val, colour = month))+
  geom_point(aes(color=month), shape = 20, size = 4)+
  geom_point(aes(x = expl.val, y = 0, color=month), shape = 20, size = 4)+
  scale_color_manual(values = P_meticulosahighPesti$color)+
  annotate("rect", xmin = min(P_meticulosahighPesti$expl.val), xmax = max(P_meticulosahighPesti$expl.val), ymin = min(P_meticulosahighPesti$pred.val), ymax = max(P_meticulosahighPesti$pred.val),
           fill = NA, color = "red", size = 1, linetype = "dashed") +
  geom_segment(aes(x = min(expl.val), xend = max(expl.val), y = 0, yend =0, linetype= "high existence probability"), data = P_meticulosahighPesti, color = "red")+
  geom_segment(aes(x = 0, xend = 0, y = 0, yend =0, linetype= "low existence probability"), data = P_meticulosahighPesti, color = "black")+
  scale_linetype_manual("turning points range",values=c("high existence probability"=2, "low existence probability"=2))+
  geom_line(aes(x = expl.val, y = pred.val*100, colour = month), data=data, linewidth = 0.8)+
  labs(x = name, y=NULL, color ="month")+
  ylim(0,80)+
...
combined_plot <-ggarrange(plotlist=plot_list, ncol =2, nrow=2, common.legend =TRUE, legend= "right")
combined_plot <- grid.arrange(combined_plot, top = "Response curve of P.meticulosa to land management invervention", left = "probability of pest existence(%)", bottom = "land management variables (kg/hectare of crops)")
plot(combined_plot)
...
folder_path <- "P_meticulosa_newmanage/modelsscore/"
file_prefix <- "P_meticulosa_modelsscore_"
...
write.csv(merged_data, file = "P_meticulosa_newmanage/P_meticulosa_eval_scores.csv", row.names = FALSE)
# Write algo data to CSV file
write.csv(algo_data, file = "P_meticulosa_newmanage/P_meticulosa_eval_scores_algo.csv", row.names = FALSE)
...
folder_path <- "P_meticulosa_nomanageeva/"
file_prefix <- "P_meticulosa_modelsscore_"
algo_data <- data.frame(run = 1:35)
merged_data <- data.frame(run = 1:35)
for (month in periods) {
...
write.csv(merged_data, file = "P_meticulosa_newmanage/P_meticulosa_eval_scoresnomanage.csv", row.names = FALSE)
write.csv(algo_data, file = "P_meticulosa_newmanage/P_meticulosa_eval_scoresnomanage_algo.csv", row.names = FALSE)

#Put variable contribution data together
# Specify the folder path and file prefix
folder_path <- "P_meticulosa_newmanage/var_importance/"
file_prefix <- "P_meticulosa_varcon_"
...
write.csv(merged_con, file = "P_meticulosa_newmanage/P_meticulosa_eval_varcon.csv", row.names = FALSE)